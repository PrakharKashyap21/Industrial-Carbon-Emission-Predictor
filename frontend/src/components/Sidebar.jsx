...
export const Sidebar = ({ isCollapsed, onToggle, isMobileOpen, onMobileClose }) => {
  const { user } = useAuth();
  ...
  return (
    <>
      {/* 1. DESKTOP SIDEBAR (FROZEN UNTOUCHED - ONLY VISIBLE ON lg: STACK) */}
      <aside
        className={`hidden lg:flex fixed top-0 left-0 z-30 h-screen bg-slate-900 text-slate-300 border-r border-slate-800 transition-all duration-300 flex-col justify-between ${
      ...
      {/* 2. MOBILE NAVIGATION DRAWER & BACKDROP (< lg SCREEN BREAKPOINT) */}
      {isMobileOpen && (
        <div
      ...
      <aside
        className={`fixed top-0 left-0 z-50 h-screen w-72 max-w-[85vw] bg-slate-900 text-slate-300 border-r border-slate-800 flex flex-col justify-between shadow-2xl transition-transform duration-300 lg:hidden ${
      ...
      >
                  ...
                  {section.items.map((item) => {
                    ...
                    return (
                      <NavLink
                        ...
                        to={item.path}
                        onClick={onMobileClose}
                        className={`flex items-center gap-3 px-3 py-3 rounded-lg text-xs font-semibold transition-all ${
                          isActive
        ...
        <div className="p-3 border-t border-slate-800 bg-slate-950/40">
          <NavLink
            to="/profile"
            onClick={onMobileClose}
            className="flex items-center gap-3 p-2 rounded-lg text-slate-300 hover:text-white hover:bg-slate-800/60 transition-colors"
          >
