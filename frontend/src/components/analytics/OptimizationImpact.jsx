    ...
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-1">
          <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">Cumulative Estimated Saving</span>
          <div className="text-2xl font-extrabold text-emerald-300 font-mono">
            {cumulative_estimated_saving_kg ? Math.round(cumulative_estimated_saving_kg).toLocaleString() : '0'} <span className="text-xs font-normal text-slate-400">kg CO₂</span>
          </div>
          <span className="text-[10px] text-slate-500 block">Total model-estimated reduction</span>
        </div>

        <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-1">
          <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block flex items-center text-cyan-400">
        ...
        <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-1">
          <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">Single Best Run Saving</span>
          <div className="text-2xl font-extrabold text-white font-mono">
            {best_run_saving_kg ? Math.round(best_run_saving_kg).toLocaleString() : '0'} <span className="text-xs font-normal text-slate-400">kg</span>
          </div>
      ...
      {history && history.length > 0 && (
        <div className="space-y-2">
          <span className="text-[11px] font-bold text-slate-300 uppercase tracking-wider block">Recent Optimization Search Logs</span>
          <div className="space-y-1.5 font-mono text-xs max-h-36 overflow-y-auto">
            {history.map((h, idx) => (
              <div key={idx} className="bg-slate-950 p-2.5 rounded-lg border border-slate-800 flex justify-between items-center text-[11px]">
                <div>
                  <span className="font-bold text-cyan-400">{h.optimization_id}</span>
                  <span className="text-slate-500 ml-2 font-sans">{h.date}</span>
                </div>
                <div className="text-right">
                  <span className="text-emerald-400 font-bold">-{h.estimated_reduction_kg} kg (-{h.estimated_reduction_percentage}%)</span>
                </div>
              </div>
            ))}
          </div>
